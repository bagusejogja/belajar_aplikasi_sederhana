...
export default function MasterUnitTabs({ activeTab }: { activeTab?: string }) {
  ...
  return (
    <div className="bg-white/95 backdrop-blur-sm p-3 px-4 md:px-5 rounded-2xl border border-gray-200/90 shadow-xs mb-4">
      ...
      {/* Navigation Grid: 3 Kolom Rata & Rapi dengan ukuran seragam */}
          ...
          return (
            <Link
              ...
              className={`group flex items-center justify-between gap-2 px-3.5 py-2.5 rounded-xl font-bold text-xs transition-all duration-200 select-none cursor-pointer border h-full min-h-[44px] ${
                isActive
                  ? 'bg-gradient-to-r from-indigo-600 via-indigo-600 to-purple-700 text-white shadow-md shadow-indigo-200/70 border-indigo-500 ring-1 ring-indigo-500/40 scale-[1.01]'
                  : 'bg-slate-50/90 hover:bg-indigo-50/70 text-slate-700 hover:text-indigo-700 border border-slate-200/90 hover:border-indigo-200 hover:shadow-2xs active:scale-[0.99]'
              }`}
            >
