...
import { 
  FileEdit, Upload, Download, FileText, Sparkles, 
...
export default function UnitKerjaDashboard() {
  ...
  const groupedData = useMemo(() => {
    if (groupBy.length === 0) return { "Total Keseluruhan": { count: filteredBudgets.length, sum: filteredBudgets.reduce((acc, b) => acc + (b.total || 0), 0) } };
  ...
  // Summary per Unit Kerja computation (REAKTIF SESUAI FILTERED DATA)
  ...
  const exportToExcelByTab = (tab: 'summary' | 'data' | 'pivot') => {
    ...
    if (tab === 'summary') {
      ...
      const dataToExport = filteredBudgets.map((b, idx) => ({
        'No': idx + 1,
        'ID DB': b.id_db || '',
      ...
      XLSX.writeFile(workbook, `Usulan_Anggaran_Unit_Kerja_${dateStr}.xlsx`);
  ...
  const generateFormattedPDFReport = (tab: 'summary' | 'data' | 'pivot') => {
    ...
    if (tab === 'summary') {
      ...
      `;
      tableRowsHTML = filteredBudgets.slice(0, 500).map((b, idx) => {
    ...
    const htmlContent = `
      ...
      <html>
      <head>
        <title>Laporan_Usulan_Anggaran_Unit_Kerja_${tab}_${new Date().toISOString().slice(0,10)}</title>
        <style>
    ...
    <div className="max-w-7xl mx-auto pb-24 space-y-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-white p-3.5 px-5 rounded-2xl border border-gray-200/80 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div className="bg-gradient-to-br from-sky-600 to-indigo-700 p-2.5 rounded-xl text-white shadow-xs">
            <FileEdit size={20} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-base font-black text-gray-900 tracking-tight leading-none">
                Portal Usulan Anggaran Unit Kerja
              </h1>
              <span className="px-2 py-0.5 rounded-md bg-sky-50 text-sky-700 border border-sky-200 text-[10px] font-black">
                Submission & Review Hub
              </span>
            </div>
            <p className="text-gray-500 font-medium text-[11px] mt-0.5">
              Pantau status penguncian anggaran (Rule & AI), ajukan usulan (Paste Zone), dan kelompokkan data multi-dimensi.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
          ...
          <button 
            ...
            className="h-9 px-3.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs active:scale-95 cursor-pointer"
          >
            <FileText size={13} />
          ...
          <button 
            ...
            className="h-9 px-3.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-2xs active:scale-95 cursor-pointer"
          >
            <Upload size={13} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        {/* CARD 4: COVERAGE PENGUNCIAN */}
        <div className="bg-white rounded-2xl p-4 border border-gray-200/80 shadow-xs flex flex-col justify-between">
          <div className="flex items-start justify-between">
            <div>
              <span className="text-[10px] font-black uppercase tracking-wider text-gray-400 block mb-1">SUMBER PENGUNCIAN</span>
        ...
        <div className="flex space-x-1 bg-gray-100 p-1 rounded-xl w-max mb-6 border border-gray-200">
          ...
          >
            📋 Tabel Data Detail ({filteredBudgets.length})
          </button>
          <button 
        ...
        {activeTab === 'data' && (
          ...
          <CardHeader>
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
              <div>
                <CardTitle className="text-xl text-gray-900">Usulan Anggaran Detail ({filteredBudgets.length} Data)</CardTitle>
                <CardDescription className="text-gray-500">
          ...
          <CardContent>
            {/* Modal Detail Usulan (Pop Up saat diklik) */}
                  ...
                  <div className="flex justify-between items-center gap-2">
                    <div>
                      <DialogTitle className="text-lg font-bold text-indigo-950">
                        Detail Usulan #{viewDetailData?.id_db}
                      </DialogTitle>
                      <DialogDescription className="text-gray-500 text-xs">
                        {viewDetailData?.unitkerja_nama} — Kode Akun: {viewDetailData?.akun}
                      </DialogDescription>
                    </div>
                    {viewDetailData?.kunci === 'Y' ? (
                ...
                {viewDetailData && (
                  <div className="space-y-4 text-xs mt-2">
                    <div className="p-3 rounded-xl bg-gray-50 border border-gray-200 space-y-2">
                      <div>
                        <span className="font-bold text-gray-500 uppercase text-[10px]">Nama Komponen</span>
                        <p className="font-semibold text-gray-900 text-sm mt-0.5">{viewDetailData.komponen_nama || '-'}</p>
                      </div>
                      <div>
                        <span className="font-bold text-gray-500 uppercase text-[10px]">Deskripsi Usulan</span>
                        <p className="font-medium text-gray-800 mt-0.5">{viewDetailData.deskripsi || '-'}</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 rounded-xl bg-indigo-50/50 border border-indigo-100">
                        <span className="font-bold text-indigo-600 uppercase text-[10px]">Lingkup</span>
                        <p className="font-semibold text-gray-900 mt-0.5">{viewDetailData.lingkup || '-'}</p>
                      </div>
                      <div className="p-3 rounded-xl bg-indigo-50/50 border border-indigo-100">
                        <span className="font-bold text-indigo-600 uppercase text-[10px]">Maksud / Tujuan</span>
                        <p className="font-semibold text-gray-900 mt-0.5">{viewDetailData.maksud_tujuan || '-'}</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-3 gap-3 p-3 rounded-xl bg-slate-900 text-white">
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold uppercase">Volume</span>
                        <p className="font-mono text-sm font-bold mt-0.5">{viewDetailData.vol || 1}</p>
                      </div>
                      <div>
                        <span className="text-[10px] text-slate-400 font-bold uppercase">Tarif (Rp)</span>
                        <p className="font-mono text-sm font-bold mt-0.5">Rp {new Intl.NumberFormat('id-ID').format(viewDetailData.tarif || 0)}</p>
                      </div>
                      <div>
                        <span className="text-[10px] text-indigo-300 font-bold uppercase">Total Usulan</span>
                        <p className="font-mono text-sm font-black text-indigo-300 mt-0.5">Rp {new Intl.NumberFormat('id-ID').format(viewDetailData.total || 0)}</p>
                      </div>
                    </div>

                    <div className="p-3 rounded-xl bg-purple-50 border border-purple-200 space-y-1">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-purple-900 uppercase text-[10px]">🤖 Analisis Keyakinan AI</span>
                        <Badge className="bg-purple-600 text-white text-[10px]">
                          {viewDetailData.ai_confidence ? `${Math.round(viewDetailData.ai_confidence * 100)}% Confidence` : 'Evaluasi Rule'}
                        </Badge>
                      </div>
                      <p className="text-purple-950 font-medium leading-relaxed">
                        {viewDetailData.ai_reason || 'Dicocokkan dengan Master Aturan penguncian anggaran.'}
                      </p>
                    </div>
                  </div>
                )}
            ...
            {/* Modal Import Data Excel (Paste Zone) */}
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogContent className="bg-white text-gray-900 border-gray-200 sm:max-w-[750px] w-full max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <div className="flex justify-between items-center gap-2">
                    <div>
                      <DialogTitle>Import Usulan Anggaran dari Excel</DialogTitle>
                      <DialogDescription className="text-gray-500">
                        Copy baris data dari Excel dan paste langsung ke kotak di bawah ini.
                      </DialogDescription>
                    </div>
                    <Button 
                      variant="outline" 
                      ...
                      onClick={() => setPasteData(`id_db\tlingkup\tmaksud_tujuan\tkomponen_nama\tunitkerja_nama\tdeskripsi\ttarif\tvol\ttotal\tunitkerja\takun
2957\tDukungan Operasional Pimpinan\tPERJALANAN DINAS DALAM NEGERI\tKetua Senat Akademik\tSenat Akademik\tPERJALANAN DINAS DALAM NEGERI -- Zona Tujuan 1\t2600000\t42\t109200000\tSenat Akademik\t54101`)}
                    >
                      💡 Isi Contoh Format Tab
                    </Button>
                  </div>
                </DialogHeader>
                ...
                <div className="space-y-4 text-xs mt-2">
                  <div className="p-3 rounded-xl bg-indigo-50/50 border border-indigo-100 space-y-1.5">
                    <p className="font-bold text-indigo-900">📋 Format Kolom Excel yang Didukung (Dipisah TAB):</p>
                    <p className="text-gray-600 font-mono text-[11px] overflow-x-auto whitespace-nowrap p-1.5 bg-white rounded-lg border border-indigo-200">
                      ID_DB | Lingkup | Maksud_Tujuan | Komponen | (kosong) | Deskripsi | Harga | Vol | Total | Unit_Kerja | Kode_Akun
                    </p>
                    <p className="text-gray-500 italic">Sistem secara otomatis akan mengevaluasi aturan (Rule Engine) & AI begitu data berhasil diimpor.</p>
                  </div>
                  ...
                  <Textarea
                    ...
                    rows={10}
                    className="font-mono text-xs bg-gray-50 border-gray-300 text-gray-900 focus:bg-white"
                  />
            ...
            <div className="rounded-2xl border border-gray-200 overflow-hidden">
              <Table>
                <TableHeader className="bg-gray-50 border-b border-gray-200">
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="text-gray-500 w-16">ID DB</TableHead>
                    <TableHead className="text-gray-500">Unit Kerja</TableHead>
                    <TableHead className="text-gray-500 w-32">Kode Akun</TableHead>
                    <TableHead className="text-gray-500">Deskripsi Usulan</TableHead>
                    <TableHead className="text-gray-500 text-right">Total (Rp)</TableHead>
                    <TableHead className="text-gray-500 text-center">Rekomendasi AI</TableHead>
                    <TableHead className="text-gray-500 text-center">Status Final</TableHead>
                    <TableHead className="text-gray-500 text-center w-24">Aksi</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-8 text-gray-400">Memuat data usulan...</TableCell>
                    </TableRow>
                  ) : filteredBudgets.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-8 text-gray-400">
                        {budgets.length === 0 ? 'Belum ada data usulan. Silakan impor dari Excel.' : 'Data tidak ditemukan untuk filter ini.'}
                      </TableCell>
                    </TableRow>
                  ) : (
                    paginatedBudgets.map((budget) => {
                      const isHighConfidencePending = budget.ai_confidence && budget.ai_confidence >= 0.75 && (budget.kunci !== 'Y' && (!budget.custom_status || budget.custom_status.trim() === ''));
                        ...
                        <TableRow 
                          ...
                          className={`border-b border-gray-100 transition-colors ${
                            isHighConfidencePending 
                              ? 'bg-purple-50/70 hover:bg-purple-100/80 border-l-4 border-l-purple-600' 
                              : budget.kunci === 'Y' 
                                ? 'bg-emerald-50/30 hover:bg-emerald-50/60' 
                                : 'hover:bg-gray-50'
                          }`}
                        >
                          <TableCell className="font-mono font-bold text-xs text-gray-900 py-3.5 px-4">
                            {budget.id_db}
                          </TableCell>
                          <TableCell className="font-semibold text-xs text-gray-800 max-w-[200px] truncate" title={budget.unitkerja_nama}>
                            🏢 {budget.unitkerja_nama}
                          </TableCell>
                          <TableCell className="font-mono text-xs text-gray-900">
                            {budget.akun}
                          </TableCell>
                          <TableCell className="text-xs text-gray-900 max-w-[300px]">
                            <div className="font-bold text-gray-900">{budget.komponen_nama}</div>
                            <div className="text-gray-500 truncate" title={budget.deskripsi}>{budget.deskripsi}</div>
                          </TableCell>
                          <TableCell className="text-right font-mono font-bold text-xs text-gray-900">
                            Rp {new Intl.NumberFormat('id-ID').format(budget.total || 0)}
                          </TableCell>
                          <TableCell className="text-center">
                            {budget.kunci_by === 'RULE' ? (
                              <div className="inline-flex flex-col items-center">
                                <Badge className="bg-indigo-100 text-indigo-800 border-indigo-200 text-[10px] font-bold">
                                  Reff: Wajib Ada
                                </Badge>
                                <span className="text-[10px] text-indigo-600 font-semibold mt-0.5">📌 Master Rule</span>
                              </div>
                            ) : budget.ai_confidence ? (
                              <div className="inline-flex flex-col items-center">
                                <Badge className={`${budget.ai_confidence >= 0.75 ? 'bg-purple-600 text-white' : 'bg-purple-100 text-purple-800'} text-[10px] font-bold`}>
                                  🤖 AI: Wajib ({Math.round(budget.ai_confidence * 100)}%)
                                </Badge>
                                {isHighConfidencePending && (
                                  <Button
                                    size="sm"
                                    onClick={() => handleQuickStatusChange(budget, 'Wajib')}
                                    className="mt-1 h-6 px-2 text-[10px] bg-purple-700 hover:bg-purple-800 text-white font-bold rounded-lg shadow-sm animate-pulse"
                                  >
                                    ⚡ Setujui AI
                                  </Button>
                                )}
                          ...
                          <TableCell className="text-center">
                            <StatusAutocomplete 
                              value={budget.custom_status} 
                              kunci={budget.kunci}
                              availableStatuses={availableStatusList}
                              onSelect={(val) => handleQuickStatusChange(budget, val)} 
                            />
                          </TableCell>
                          <TableCell className="text-center">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => setViewDetailData(budget)}
                              className="text-xs text-indigo-600 hover:text-indigo-900 hover:bg-indigo-50 font-semibold px-2.5 h-8"
                            >
                              🔍 Detail
                            </Button>
                          </TableCell>
              ...
              {/* PAGINASI MODERN (100 - 250 - 500 - ALL) */}
              {filteredBudgets.length > 0 && (
                <div className="p-4 bg-gray-50 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4 text-xs">
                  <div className="flex items-center gap-3 text-gray-600 font-medium">
                    ...
                    <span>
                      Menampilkan {pageSize === 'ALL' ? filteredBudgets.length : Math.min((currentPage - 1) * Number(pageSize) + 1, filteredBudgets.length)} - {pageSize === 'ALL' ? filteredBudgets.length : Math.min(currentPage * Number(pageSize), filteredBudgets.length)} dari <strong>{filteredBudgets.length}</strong> usulan
                    </span>
                  </div>
                  ...
                  {pageSize !== 'ALL' && totalPages > 1 && (
                    <div className="flex items-center gap-1.5">
                      <Button
                        ...
                        className="h-8 px-2.5 text-xs font-bold border-gray-300"
                      >
                        &laquo; Awal
                      </Button>
                      <Button
