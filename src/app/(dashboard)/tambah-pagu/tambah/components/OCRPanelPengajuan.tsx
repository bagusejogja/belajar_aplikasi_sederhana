...
export function parseOCRPengajuan(ocrText: string) {
  if (!ocrText) return {};

  let no_surat_pengajuan = '';
  let tanggal_surat_pengajuan = '';
  let hal_surat_pengajuan = '';
  let nominal_diajukan = 0;

  // 1. Parse No Surat
  const noSuratMatch = ocrText.match(/(?:nomor|no)[.\s]*(?::|;)?\s*([A-Za-z0-9/.\-_]+)/i);
  if (noSuratMatch) {
    no_surat_pengajuan = noSuratMatch[1].trim();
  }
  ...
  if (dateMatchIndo) {
    ...
    tanggal_surat_pengajuan = `${year}-${month}-${day}`;
  } else {
    const dateMatchNum = ocrText.match(numericDateRegex);
    if (dateMatchNum) {
      if (dateMatchNum[1] && dateMatchNum[1].length === 4) {
        tanggal_surat_pengajuan = `${dateMatchNum[1]}-${dateMatchNum[2].padStart(2, '0')}-${dateMatchNum[3].padStart(2, '0')}`;
      } else if (dateMatchNum[6] && dateMatchNum[6].length === 4) {
        tanggal_surat_pengajuan = `${dateMatchNum[6]}-${dateMatchNum[5].padStart(2, '0')}-${dateMatchNum[4].padStart(2, '0')}`;
      }
    }
  }
  ...
  if (perihalMatch) {
    ...
    hal_surat_pengajuan = rawPerihal;
  } else {
    ...
    if (simplePerihal) {
      ...
      hal_surat_pengajuan = rawP;
    }
  }

  // 4. Parse Nominal Diajukan
  ...
  return { no_surat_pengajuan, tanggal_surat_pengajuan, hal_surat_pengajuan, nominal_diajukan };
}

export default function OCRPanelPengajuan({ mainData, setMainData, setExternalFile, listUnit = [], onOcrComplete }: any) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setFile(f);
      setExternalFile(f); // Pass to parent for form submit
      setIsDone(false);
      setOcrText('');
      setProgress(0);
      setIsUploading(true);
      ...
      try {
        if (mainData.link_surat_pengajuan) {
          fetch('/api/upload', {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ url: mainData.link_surat_pengajuan })
          }).catch(err => console.error("Temporary R2 deletion error:", err));
        }

        const formData = new FormData();
        formData.append('file', f);
        formData.append('folder', 'pengajuan');
        ...
        if (data.success) {
           setMainData((prev: any) => ({ ...prev, link_surat_pengajuan: data.publicUrl, file_surat_pengajuan: f.name }));
  ...
  const processOCR = async () => {
    ...
    try {
      ...
      const formattedUnits = listUnit.map((u: any) => ({ id: u.value, nama_unit: u.label }));
      ...
      alert('Ekstraksi teks selesai! Metadata pengajuan usulan telah terisi.');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-4 text-emerald-800">
        ...
        <h3 className="text-lg font-black tracking-tight">Fitur Cerdas AI Ekstraksi Pengajuan</h3>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
            Upload File / Link Surat Pengajuan
          </label>
          <div 
