...
function hitungBulanAktif(tanggalLahir: string | null, status: string | null) {
  if (!tanggalLahir) return { bulan: 0, tglPensiun: '-', isPensiun2026: false, totalBulanBayar: 0, detailBulanan: Array(14).fill(false) };
  
  const birthDate = new Date(tanggalLahir);
  const pensiunYear = birthDate.getFullYear() + KONSTANTA.USIA_PENSIUN_GB;
  ...
  if (pensiunYear < KONSTANTA.TAHUN_REFERENSI) {
    ...
    bulan = 12;
    detailBulanan = Array(14).fill(true);
  } else {
    bulan = pensiunMonth;
    isPensiun2026 = true;
    for (let i = 0; i < 12; i++) {
...
export default function TunjanganGuruBesarPage() {
    ...
    return dataPegawai
      ...
      .map((p, idx) => {
        ...
        return {
          ...
          total,
          isPegawaiUGM,
          tglSertifikasi: p.tanggal_sertifikasi_gb ? new Date(p.tanggal_sertifikasi_gb).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '-'
        };
      });
  }, [dataPegawai]);
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const generateRow = (item: any, idx: number, status: string) => {
        const blnValues = item.info.detailBulanan.map((active: boolean) => active ? item.tunjGuruBesar : 0);
        return `
   <Row>
      ...
      const generateSubtotal = (list: any[], label: string) => {
        ...
        list.forEach(p => {
          p.info.detailBulanan.forEach((active: boolean, i: number) => {
            if (active) monthlyTotals[i] += p.tunjGuruBesar;
          });
        });
        ...
        return `
   <Row ss:Height="20">
    <Cell ss:MergeAcross="7" ss:StyleID="sTotalLabel"><Data ss:Type="String">SUB-TOTAL ${label} (${list.length} Guru Besar)</Data></Cell>
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
 ...
 <Worksheet ss:Name="Tunjangan GB 2026">
  <Table ss:DefaultRowHeight="15.5">
   ...
   <Row ss:Height="20"><Cell ss:MergeAcross="22" ss:StyleID="sJudul"><Data ss:Type="String">REKAPITULASI DATA PENERIMA TUNJANGAN PROFESI DOSEN GURU BESAR</Data></Cell></Row>
   ...
   <Row ss:Height="25">
    ...
    <Cell ss:StyleID="sHeader"><Data ss:Type="String">Nama Guru Besar</Data></Cell>
   ...
   <Row ss:Height="25">
    ...
    ${bulanNames.map((_, i) => {
      const totalBln = filteredData.reduce((sum, p) => sum + (p.info.detailBulanan[i] ? p.tunjGuruBesar : 0), 0);
      ...
      {!isLoading && (
        <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-gradient-to-br from-indigo-900 via-slate-900 to-indigo-950 p-8 rounded-[2.5rem] shadow-2xl text-white">
            ...
            <div className="mt-2 text-[9px] font-medium text-indigo-300 italic">* Termasuk Tunjangan 13 & 14</div>
          </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-[10px] text-slate-400 uppercase bg-slate-50 font-black tracking-widest border-b">
              <tr>
                ...
                <th className="px-6 py-6">Nama Guru Besar</th>
                <th className="px-6 py-6 text-center">Status</th>
                <th className="px-6 py-6 text-center">Gol</th>
                <th className="px-6 py-6">Tgl Pensiun (70th)</th>
                <th className="px-6 py-6">Unit Kerja</th>
                <th className="px-6 py-6 text-center">Bln</th>
                <th className="px-6 py-6 text-right">Tunj. Guru Besar</th>
              ...
              {isLoading ? (
                ...
                <tr key={item.id} className="border-b border-slate-50 hover:bg-indigo-50/20 transition-colors group">
                  ...
                  <td className="px-6 py-5 text-center">
                    <span className={`px-2 py-1 rounded-lg text-[10px] font-black ${item.isPegawaiUGM ? 'bg-emerald-50 text-emerald-700' : 'bg-indigo-50 text-indigo-700'}`}>
                  ...
                  <td className="px-6 py-5 text-center font-bold text-indigo-900">{item.info.totalBulanBayar}</td>
                  <td className="px-6 py-5 text-right font-medium text-slate-600">{formatRupiah(item.tunjGuruBesar)}</td>
