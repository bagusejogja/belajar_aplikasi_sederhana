...
function hitungBulanFungsional(tanggalLahir: string | null, jabatan: string | null, status: string | null) {
  ...
  if (bulan > 0) {
    ...
    // Sesuai aturan Serdos kemarin: PNS & Dosen Biasa dapat 13 & 14
    if (isPNS && !isGB) {
...
export default function TunjanganFungsionalPage() {
  ...
  const processedData = useMemo(() => {
    return dataPegawai
      ...
      .map((p, idx) => {
        const info = hitungBulanFungsional(p.tanggal_lahir, p.jabatan, p.status);
        
        // Tunjangan Fungsional menggunakan field tunjangan_fungsional dari tabel
        const tunjFungsional = p.tunjangan_fungsional || 0;
        const total = info.totalBulanBayar * tunjFungsional;
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const generateRow = (item: any, idx: number, status: string) => {
        const blnValues = item.info.detailBulanan.map((active: boolean) => active ? item.tunjFungsional : 0);
        return `
   <Row>
      ...
      const generateSubtotal = (list: any[], label: string) => {
        ...
        list.forEach(p => {
          p.info.detailBulanan.forEach((active: boolean, i: number) => {
            if (active) monthlyTotals[i] += p.tunjFungsional;
          });
        });
        ...
        return `
   <Row ss:Height="20">
    <Cell ss:MergeAcross="6" ss:StyleID="sTotalLabel"><Data ss:Type="String">SUB-TOTAL ${label} (${list.length} Dosen)</Data></Cell>
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
 ...
 <Worksheet ss:Name="Tunjangan Fungsional 2026">
  <Table ss:DefaultRowHeight="15.5">
   <Column ss:Width="30"/><Column ss:Width="130"/><Column ss:Width="200"/><Column ss:Width="50"/><Column ss:Width="80"/><Column ss:Width="100"/><Column ss:Width="150"/>
   ${bulanNames.map(() => `<Column ss:Width="80"/>`).join('')}
   <Column ss:Width="110"/>

   <Row ss:Height="20"><Cell ss:MergeAcross="21" ss:StyleID="sJudul"><Data ss:Type="String">REKAPITULASI KEBUTUHAN TUNJANGAN FUNGSIONAL DOSEN</Data></Cell></Row>
   <Row ss:Height="20"><Cell ss:MergeAcross="21" ss:StyleID="sJudul"><Data ss:Type="String">TAHUN ANGGARAN ${KONSTANTA.TAHUN_REFERENSI}</Data></Cell></Row>
   <Row ss:Height="15"/>
   ...
   <Row ss:Height="20"><Cell ss:MergeAcross="21" ss:StyleID="sSubJudul"><Data ss:Type="String">KELOMPOK PNS</Data></Cell></Row>
   ${pnsList.map((item, idx) => generateRow(item, idx, 'PNS')).join('')}
   ${generateSubtotal(pnsList, 'PNS')}

   <Row ss:Height="15"/>
   <Row ss:Height="20"><Cell ss:MergeAcross="21" ss:StyleID="sSubJudul"><Data ss:Type="String">KELOMPOK PEGAWAI UGM</Data></Cell></Row>
   ${ugmList.map((item, idx) => generateRow(item, idx, 'UGM')).join('')}
   ${generateSubtotal(ugmList, 'UGM')}

   <Row ss:Height="15"/>
   <Row ss:Height="25">
    <Cell ss:MergeAcross="6" ss:StyleID="sGrandTotal"><Data ss:Type="String">GRAND TOTAL KESELURUHAN</Data></Cell>
    ${bulanNames.map((_, i) => {
      const totalBln = filteredData.reduce((sum, p) => sum + (p.info.detailBulanan[i] ? p.tunjFungsional : 0), 0);
  ...
  <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
   <PageSetup><Layout x:Orientation="Landscape"/></PageSetup>
      ...
      link.setAttribute('download', `Tunjangan_Fungsional_2026.xls`);
  ...
  return (
    <div className="p-8 max-w-full mx-auto bg-slate-50 min-h-screen">
      <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none">Tunjangan Fungsional</h1>
      ...
      {!isLoading && (
        <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between min-w-0">
             <div className="min-w-0 flex-1 pr-4">
               <span className="text-[10px] font-black uppercase text-slate-400 mb-1 block">PNS</span>
      ...
      <div className="mb-6 px-8 py-4 bg-blue-50 border border-blue-100 rounded-2xl text-blue-800 text-xs italic font-medium flex items-center gap-3">
        ...
        Note: Tunjangan Fungsional menggunakan field tunjangan_fungsional. Batas usia pensiun mengikuti Dosen: Dosen Biasa 65th, Guru Besar 70th.
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-[10px] text-slate-400 uppercase bg-slate-50 font-black tracking-widest border-b">
              <tr>
                ...
                <th className="px-6 py-6">Profil Pegawai</th>
                <th className="px-6 py-6 text-center w-24">Bln</th>
                <th className="px-6 py-6 text-right w-48">Tunj. Fungsional</th>
                ...
                <tr key={item.id} className="border-b border-slate-50 hover:bg-blue-50/20 transition-colors group">
                  ...
                  <td className="px-6 py-5 align-top">
                    <div className="flex flex-col gap-1.5">
                      <div className="font-bold text-sm text-slate-800 group-hover:text-blue-600 transition-colors">{item.nama_pegawai}</div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider ${item.isPegawaiUGM ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-blue-50 text-blue-700 border border-blue-100'}`}>
                  ...
                  <td className="px-6 py-5 text-center font-bold text-blue-900 align-top">{item.info.totalBulanBayar}</td>
                  <td className="px-6 py-5 text-right font-medium text-slate-600 align-top">{formatRupiah(item.tunjFungsional)}</td>
